(() => {
			...
			return (
				...
				parseFloat(
					window
						...
						.getPropertyValue('width' === size ? 'margin-right' : 'margin-top')
				) +
				parseFloat(
					window
						.getComputedStyle(el, null)
						.getPropertyValue('width' === size ? 'margin-left' : 'margin-bottom')
				)
			);
	...
	function updateProgress(translate) {
		...
		if (params.loop) {
			const firstSlideIndex = utils_elementIndex(
				swiper.slides.filter(el => '0' === el.getAttribute('data-swiper-slide-index'))[0]
			);
			const lastSlideIndex = utils_elementIndex(
				swiper.slides.filter(
					el =>
						1 * el.getAttribute('data-swiper-slide-index') === swiper.slides.length - 1
				)[0]
			);
	...
	function slideToLoop(
		...
		if (swiper.params.loop)
			if (swiper.virtual && swiper.params.virtual.enabled)
				...
				newIndex = utils_elementIndex(
					swiper.slides.filter(
						slideEl => 1 * slideEl.getAttribute('data-swiper-slide-index') === newIndex
					)[0]
				);
		...
		if (params.loop) {
			...
			if (params.centeredSlides)
				if (
					...
					swiper.loopFix();
					slideToIndex = utils_elementIndex(
						utils_elementChildren(
							slidesEl,
	...
	function loopFix({
		...
		if (swiper.virtual && params.virtual.enabled) {
			if (slideTo)
				if (!params.centeredSlides && 0 === swiper.snapIndex)
		...
		if ('undefined' === typeof activeSlideIndex)
			activeSlideIndex = utils_elementIndex(
	...
	function loopDestroy() {
		...
		slides.forEach(slideEl => {
			const index =
				'undefined' === typeof slideEl.swiperSlideIndex
		...
		mount(element) {
			...
			Object.assign(swiper, {
				...
				rtlTranslate:
					...
					('rtl' === el.dir.toLowerCase() || 'rtl' === elementStyle(el, 'direction')),
	...
	(function () {
		...
		const enableSwiper = function () {
			if (document.querySelector('.media__slider'))
				mySwiper = new core('.media__slider', {
					modules: [Navigation],
					observer: true,
					observeParents: true,
					slidesPerView: 3,
			...
			if (document.querySelector('.blog__slider'))
				mySwiper1 = new core('.blog__slider', {
					modules: [Navigation],
					observer: true,
					observeParents: true,
					slidesPerView: 3,
